...
class PeriodicScenarioRunner implements EventEmitterInterface
{
    ...
    protected function sendSlot(): void
    {
        ...
        $this->nextSlot++;
        // $this->logger->debug('Next slot: ' . var_export($this->nextSlot, true) . ' / ' . var_export($this->slotCount, true));
        ...
        try {
            ...
            if ($method === 'get') {
                ...
                try {
                    ...
                    // $this->logger->notice(sprintf('WALKING %d tables', count($tables)) . implode(', ', array_keys($tables)));
                } catch (Throwable $e) {
            ...
            EventLoop::queue(function () use ($deferred, $e) {
                $deferred->reject($e);
            });
    ...
    protected function processResult($result, PeriodicScenarioSingleRequest $request): void
    {
        ...
        try {
            ...
            $this->logger->notice('Processing Result failed: ' . $e->getMessage());
        }
    }
    ...
    protected function processFailure(Exception $reason, PeriodicScenarioSingleRequest $request): void
    {
        ...
        try {
            $this->logger->error("Scenario failed for $address: " . $reason->getMessage());
            $this->emit(
                self::ON_RESULT,
            ...
            $this->logger->error('Error on Scenario result handling: ' . $e->getMessage());
        }
    }
    ...
    {
        $this->enqueuingTimer ??= EventLoop::repeat(self::DELAY_BETWEEN_REQUEST_BATCHES, function () {
            $this->enqueueNextRequests(self::REQUESTS_TO_LAUNCH_AT_ONCE);
        });
    }
