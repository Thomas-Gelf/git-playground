...
use IcingaFeature\Inventory\Db\ComponentWithDb;
use IcingaFeature\Inventory\Db\DbQueryHelper;
use IcingaFeature\Inventory\Db\SimpleDbBasedComponent;
...
class SnmpTaskShipper implements ComponentWithDb
{
    use SimpleDbBasedComponent;

    protected UuidInterface $dataNodeUUid;

    public function __construct(
        protected readonly Feature $inventoryFeature,
        protected readonly LoggerInterface $logger,
    ) {
        $this->dataNodeUUid = $this->inventoryFeature->nodeIdentifier->uuid;
    }

    public function ship(): void
    {
        if ($this->db === null) {
            // TODO: getDb, which logs once on no DB and once, when back?
            return;
        }
        if ($rpcHandler = $this->getLocalSnmpRpcHandler()) {
            $this->logger->notice('I HAVE SNMP');
            $localCredentials = CredentialLoader::fetchAllForDataNode($this->dataNodeUUid, $this->db);
    ...
    {
        foreach ($this->inventoryFeature->getRegisteredRpcNamespaces() as $registeredRpcNamespace => $rpcHandler) {
            if ($registeredRpcNamespace === 'snmp' && method_exists($rpcHandler, 'setCredentialsRequest')) {
                return $rpcHandler;
            }
        }
        $this->logger->notice('NO SNMP'); // TODO: Log once
