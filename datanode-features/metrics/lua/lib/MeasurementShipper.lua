...
MeasurementShipper.new = function(inventory, queue)
  ...
  function self.pushMeasurement(jsonString)
    ...
    if err then
      inventory.deferMeasurementForMissingDs(ci, ciConfig, jsonString)
  ...
  -- TODO: unused argument ciConfigs, accessing undefined variable ciList -> list VS configs?!
  function self.reRunDeferred(ciList)
    local jsonString, perfData, update, deferredLines
    for _, ci in ipairs(ciList) do
