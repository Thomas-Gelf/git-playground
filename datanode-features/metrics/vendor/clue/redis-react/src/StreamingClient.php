...
class StreamingClient extends EventEmitter implements Client
{
    ...
    public function close()
    {
        ...
        while ($this->requests) {
            $request = array_shift($this->requests);
            $name = array_shift($this->requestNames);
            assert($request instanceof Deferred);

            if ($remoteClosed) {
                $request->reject(new \RuntimeException(
                    'Connection closed by peer (ECONNRESET)' . $name,
                    defined('SOCKET_ECONNRESET') ? SOCKET_ECONNRESET : 104
                ));
            } else {
                $request->reject(new \RuntimeException(
                    'Connection closing (ECONNABORTED)' . $name,
