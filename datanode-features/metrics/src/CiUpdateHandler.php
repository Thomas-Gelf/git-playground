...
class CiUpdateHandler
{
    ...
    public function run()
    {
        ...
        $this->fetchLastPosition()->then($this->processNextScheduledBatch(...));
    }
    ...
    protected function processNextScheduledBatch(): void
    {
        $this->readNextBatch()->then(function ($stream) {
            ...
            $this->logger->error('Reading next CI batch failed: ' . $e->getMessage());
        });
    }
     ...
     * @param $stream
     */
    protected function processBulk($stream): void
    {
        if (empty($stream)) {
        ...
        $this->logger->debug('Setting CI update position: ' . $this->position);
        $this->redisApi->setLastCiUpdatePosition($this->position)->catch(function (Exception $e) {
            $this->logger->error(
                'Failed to set last CI update position: ' . $e->getMessage()
            );
        });

        sleep(10)->then($this->processNextScheduledBatch(...));
    }
    ...
    {
        $blockMs = '1000';
        return $this->redisApi->fetchBatchFromCiUpdateStream($this->position, 1000, $blockMs)->catch(function () {
            $this->logger->notice('Got no next CI update stream, returning an empty array');
            return [];
        });
    }
}
