...
namespace IcingaDataNode\Metrics;
...
use IcingaDataNode\Metrics\InfluxDb\InfluxDbWriterV1;
use IcingaDataNode\Metrics\InfluxDb\LineProtocol;
...
class InfluxDbStreamer
{
    ...
    protected function processStreamResult($result): void
    {
        foreach ($result as $row) {
            $this->position = $row[0];
            $value = RedisUtil::makeArray($row[1]);
