...
class ProcessRunner extends EventEmitter
{
    ...
    /**
     * @deprecated: doesn't work w/o bash, see below
     */
    ...
    public function terminate(int $timeout = 2): PromiseInterface
    {
        if ($this->terminating) {
    ...
    protected function runAgain(int $delay = 0): void
    {
        if ($this->terminating) {
    ...
    protected function scheduleRestart(string $reason, int $delay): void
    {
    ...
    public function run(): void
    {
        $cmd = 'exec ';
        if ($this->processName) {
            // TODO: -a only works with exec in bash
            // $cmd .= "-a " . escapeshellarg($this->processName) . ' ';
        }
        $cmd .= $this->command . $this->getArgumentString();
