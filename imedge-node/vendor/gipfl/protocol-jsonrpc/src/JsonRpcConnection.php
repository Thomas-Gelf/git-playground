...
class JsonRpcConnection implements LoggerAwareInterface
{
    ...
    public function __construct(DuplexStreamInterface $connection, JsonRpcHandler $handler = null)
    {
        ...
        $this->connection->on('data', function ($data) {
            try {
                ...
                $response = new Response();
                $response->setError(Error::forException(new Exception($error->getMessage(), $error->getCode(), $error)));
                $this->connection->write($response->toString());
            }
        });
    ...
    {
        // $this->logger->notice('< ' . spl_object_id($this) . $packet->toString());
//        file_put_contents('/tmp/jsonrpc-' . \posix_getpid() . '.json', '< ' . $packet->toString() . "\n", FILE_APPEND);
        if ($packet instanceof Response) {
            $this->handleResponse($packet);
        } elseif ($packet instanceof Request) {
            if ($this->handler) {
                $result = $this->handler->processRequest($packet, $this);
            } else {
            ...
            if ($this->handler) {
                $this->handler->processNotification($packet, $this);
            }
        } else {
    ...
    public function sendRequest(Request $request)
    {
        ...
        try {
            // $this->logger->notice('> ' . spl_object_id($this) . $request->toString());
//            file_put_contents('/tmp/jsonrpc-' . \posix_getpid() . '.json', '> ' . $request->toString() . "\n", FILE_APPEND);
            $this->connection->write($request->toString());
        } catch (JsonEncodeException $e) {
            // $this->logger->error('..' . $e->getMessage() . ': ' . iconv('UTF-8', 'UTF-8//IGNORE', print_r($request->jsonSerialize(), 1)));
            return reject($e->getMessage());
        } catch (\Throwable $e) {
            // $this->logger->error('WWWW' . $e->getMessage());
            return reject($e->getMessage());
        }
    ...
    {
        try {
//            $this->logger->notice("request $method: " . var_export(JsonString::encode($params)));
        } catch (\Throwable $e) {
