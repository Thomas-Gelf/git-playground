...
class NamespacedPacketHandler implements JsonRpcHandler
{
    ...
    public function processNotification(Notification $notification, JsonRpcConnection $connection)
    {
        ...
        try {
            $this->call($namespace, $method, $notification, $connection);
    ...
    public function processRequest(Request $request, JsonRpcConnection $connection)
    {
        ...
        try {
            return $this->call($namespace, $method, $request, $connection);
        } catch (Exception $exception) {
    ...
    protected function call($namespace, $method, Notification $notification, JsonRpcConnection $connection)
    {
        if (! isset($this->handlers[$namespace])) {
            // echo "Not found - $namespace - $method " . implode(' - ', array_keys($this->handlers)).  " \n";
        ...
        if ($handler instanceof JsonRpcHandler) {
            if ($notification instanceof Request) {
                return $handler->processRequest($notification, $connection);
            } else {
                $handler->processNotification($notification, $connection);
            }
        }
        ...
        if (! is_array($params)) {
            try {
                $params = $this->prepareParams($notification->getMethod(), $params, $connection);
            } catch (Exception $e) {
    ...
    protected function prepareParams($method, $params, JsonRpcConnection $connection)
    {
        ...
        foreach ($meta->getParameters() as $parameter) {
            ...
            if (property_exists($params, $name)) {
                ...
                $type = $parameter->getType();
                if (class_exists($type) && ltrim($type, '\\') === JsonRpcConnection::class) {
                    $result[] = $connection;
                } else {
