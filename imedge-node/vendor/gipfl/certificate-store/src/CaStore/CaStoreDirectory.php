...
namespace gipfl\CertificateStore\CaStore;
...
use gipfl\CertificateStore\DiskBased\FileStore;
...
class CaStoreDirectory implements CaStoreInterface, DirectoryBasedComponent, FileStore
{
    ...
    /*
TODO: remove this, it's just for me

The ssldir has the following structure. See the Configuration reference for details about each puppet.conf setting
listed:

ca directory (on the CA master only): Contains the files used by Puppet’s certificate authority. Mode: 0755.
  Setting: cadir.
ca_crl.pem: The master copy of the certificate revocation list (CRL) managed by the CA. Mode: 0644. Setting: cacrl.
ca_crt.pem: The CA’s self-signed certificate. This cannot be used as a master or agent certificate; it can only be used
   to sign certificates. Mode: 0644. Setting: cacert.
ca_key.pem: The CA’s private key, and one of the most security-critical files in the Puppet certificate infrastructure.
   Mode: 0640. Setting: cakey.
        ca_pub.pem: The CA’s public key. Mode: 0644. Setting: capub.


inventory.txt: A list of the certificates the CA signed, along with their serial numbers and validity periods.
  Mode: 0644. Setting: cert_inventory.
requests (directory): Contains the certificate signing requests (CSRs) that have been received but not yet signed. The
   CA deletes CSRs from this directory after signing them. Mode: 0755. Setting: csrdir.
<name>.pem: CSR files awaiting signing.
serial: A file containing the serial number for the next certificate the CA signs. This is incremented with each new
   certificate signed. Mode: 0644. Setting: serial.
