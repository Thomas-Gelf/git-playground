...
class TimePeriodSlot
{
    ...
    protected static function generateNext(string $expression, DTI $after, DTI $to): ?DT
    {
        ...
        if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $expression)) {
            ...
            if (preg_match('/^(?:mon|tues|wednes|thurs|fri|satur|sun)day$/i', $expression)) {
                $next = $datetime->modify("next $expression");
            } elseif (preg_match('/^(january|february|march|april|may|june|july|august|september|october|november|december)\s(\d{1,2})$/i', $expression)) {
